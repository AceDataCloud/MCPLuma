"""Configuration management for MCP Luma server."""
...
class Settings:
    ...
    default_aspect_ratio: str = field(
        default_factory=lambda: os.getenv("LUMA_DEFAULT_ASPECT_RATIO", "16:9")
    )

    # Request Configuration
    request_timeout: float = field(
        default_factory=lambda: float(os.getenv("LUMA_REQUEST_TIMEOUT", "180"))
    )

    # Server Configuration
    server_name: str = field(default_factory=lambda: os.getenv("MCP_SERVER_NAME", "luma"))
